...
class EncodeThread(QThread):
    ...
    # CHANGED: thêm tham số max_size_mb để nhận giá trị dung lượng động từ giao diện
    def __init__(self, input_folder, output_folder, max_size_mb):
        super().__init__()
        self.input_folder = Path(input_folder)
        self.output_folder = Path(output_folder)
        self.max_size_mb = max_size_mb  # CHANGED: lưu giá trị dung lượng được truyền vào
    ...
    def run(self):
        try:
            ...
            errors = []
            successes = []
            MAX_SIZE = int(self.max_size_mb * 1024 * 1024)  # CHANGED: sử dụng giá trị động thay vì cố định 35MB
